...
WebUI.setText(findTestObject('Session List/sessionlist_search input'), 'copy')
...
'再確認搜尋出的結果會顯示第1個session'
WebUI.waitForElementPresent(findTestObject('Session List/sessionlist_first_session_owner(Only one session)'), 3000)

WebUI.verifyElementText(findTestObject('Session List/sessionlist_first session name(only one session)'), 'copy')

WebUI.click(findTestObject('Session List/sessionlist_session_settings'))

WebUI.click(findTestObject('Session List/sessionlist_session_Make a Copy'))

WebUI.click(findTestObject('Session List/sessionlist_session_Make a Copy_copy'))

WebUI.waitForElementPresent(findTestObject('Session List/sessionlist_first_session_name(over one session)'), 3000)
